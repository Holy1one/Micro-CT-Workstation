...
- 客户区第一行只保留紧凑的可展开工程菜单，设计高度 26px。禁止增加 MicroCT/MICROCT 品牌标识、Logo 和重复的 Production/Production workstation 文案；原生窗口标题保留应用名。主题、连接状态和任务信息放在状态栏。
...
- 日志栏默认高度为 270 个设计像素；增加日志高度时只压缩同列的 Scan Parameters 和 Equipment View，保持列宽、排列和右侧通栏布局。控制坞按钮可压缩高度，但图标必须保持原有宽高比。
