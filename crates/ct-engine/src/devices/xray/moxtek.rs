//! Moxtek X-ray source driver for the current PC-direct control topology.
//!
//! The driver owns binary serial framing, source discovery, verified readback,
//! and fail-closed beam shutdown. It does not decide when exposure is allowed;
//! the scan coordinator applies that safety policy before calling this module.
...
const VOLTAGE_READBACK_TOLERANCE_KV: f64 = 0.3;
const CURRENT_READBACK_TOLERANCE_UA: f64 = 5.0;
...
pub struct XrayHealth {
    ...
    pub telemetry_warning: Option<String>,
...
impl MoxtekAdapter {
    ...
    /// Verifies active emission continuously. Lost output flags, setpoint
    /// changes, interlock opening, over-temperature, and communication errors
    /// fail closed. Measured-channel drift is telemetry only: it is returned as
    /// a detailed warning while the device-accepted setpoint remains authoritative.
    pub fn refresh_emission_status(&mut self) -> Result<XrayHealth, XrayError> {
        let result: Result<(Setpoints, Status, Option<String>), XrayError> = (|| {
            let (requested_voltage, requested_current) =
                self.requested_setpoint.ok_or_else(|| {
                    XrayError::Safety(
                        "active emission has no setpoint verified in this session".into(),
                    )
                })?;
            let setpoints = self.read_setpoints()?;
            let status = self.read_status()?;
            if !setpoints.enabled || !status.kv_enabled || !status.ua_enabled {
                return Err(XrayError::Safety(
                    "Moxtek active emission lost one or more output flags".into(),
                ));
            }
            if status.locked {
                return Err(XrayError::Safety(
                    "Moxtek digital interlock opened during emission".into(),
                ));
            }
            if !status.temperature_c.is_finite()
                || status.temperature_c > MAX_CASE_TEMPERATURE_C
            {
                return Err(XrayError::Safety(format!(
                    "Moxtek case temperature {:.2} C exceeds {:.0} C during emission",
                    status.temperature_c, MAX_CASE_TEMPERATURE_C
                )));
            }
            let set_voltage_error = (setpoints.voltage_kv - requested_voltage).abs();
            let set_current_error = (setpoints.current_ua - requested_current).abs();
            let measured_voltage_error = (status.voltage_kv - requested_voltage).abs();
            let measured_current_error = (status.current_ua - requested_current).abs();
            if set_voltage_error > VOLTAGE_READBACK_TOLERANCE_KV
                || set_current_error > CURRENT_READBACK_TOLERANCE_UA
            {
                return Err(XrayError::Safety(format!(
                    "Moxtek accepted setpoint changed during emission: requested={requested_voltage:.3} kV/{requested_current:.3} uA, set={:.3} kV/{:.3} uA, errors={:.3} kV/{:.3} uA, limits {:.3} kV/{:.3} uA",
                    setpoints.voltage_kv,
                    setpoints.current_ua,
                    set_voltage_error,
                    set_current_error,
            ...
            let telemetry_warning = if measured_voltage_error > VOLTAGE_READBACK_TOLERANCE_KV
                || measured_current_error > CURRENT_READBACK_TOLERANCE_UA
            {
                Some(format!(
                    "Moxtek measured telemetry outside advisory limits: requested={requested_voltage:.3} kV/{requested_current:.3} uA, measured={:.3} kV/{:.3} uA, errors={measured_voltage_error:.3} kV/{measured_current_error:.3} uA, advisory limits {:.3} kV/{:.3} uA; accepted setpoint unchanged",
                    status.voltage_kv,
                    status.current_ua,
                    VOLTAGE_READBACK_TOLERANCE_KV,
                    CURRENT_READBACK_TOLERANCE_UA
            ...
            Ok((setpoints, status, telemetry_warning))
        })();
        ...
        match result {
            Ok((setpoints, status, telemetry_warning)) => {
                ...
                self.health.current_ua = Some(status.current_ua);
                self.health.temperature_c = Some(status.temperature_c);
                self.health.telemetry_warning = telemetry_warning;
                self.health.last_error = None;
                Ok(self.health())
            }
            Err(error) => {
                if self.force_off_inner().is_err() {
                    self.health.beam_off_confirmed = false;
                }
                self.health.last_error = Some(error.to_string());
                Err(error)
    ...
    /// (manufacturer command 0x74). Hardware verification shows that an armed
    /// timer bounds continuous emission by the configured delay, including a
    /// host that stops responding. Releasing is only allowed while OFF; every
    /// subsequent beam enable re-arms the timer before output.
    pub fn set_usb_auto_shutdown(&mut self, enabled: bool) -> Result<XrayHealth, XrayError> {
        if !enabled && self.health.beam_on {
            return Err(XrayError::Safety(
                "USB auto-shutdown cannot be released while the beam is ON".into(),
            ));
        }
        self.set_usb_auto_shutdown_state(enabled)?;
        Ok(self.health())
    }
    ...
    fn connect_transport(
        ...
        self.transport = Some(std::sync::Arc::new(std::sync::Mutex::new(
            transport,
        )));
        ...
        // Always command and verify OFF during connection. This also recovers
        // immediately when a previous process was killed while the tube was
        // emitting; merely reporting that inherited ON state is not safe.
    ...
    /// Connection setup uses this evidence and then unconditionally commands
    /// and verifies OFF before the adapter is returned to the engine.
    ...
    fn beam_on_inner(&mut self, cancel: &AtomicBool) -> Result<(), XrayError> {
        ...
        let enable_ack_warning = match self.transact(CMD_SET_XRAY_ENABLE, &[1], 4) {
            ...
            Err(error) if is_transport_timeout(&error) => Some(format!(
                "Moxtek ON acknowledgement timed out; output accepted only after independent 0x43/0x80 verification: {error}"
            )),
        ...
        loop {
            ...
            let stable = setpoints.enabled
                && status.kv_enabled
                && status.ua_enabled
                && (setpoints.voltage_kv - requested_voltage).abs()
                    <= VOLTAGE_READBACK_TOLERANCE_KV
                && (setpoints.current_ua - requested_current).abs()
                    <= CURRENT_READBACK_TOLERANCE_UA;
            ...
            if stable_samples >= 2 {
                ...
                let measured_voltage_error = (status.voltage_kv - requested_voltage).abs();
                let measured_current_error = (status.current_ua - requested_current).abs();
                self.health.telemetry_warning = enable_ack_warning.clone().or_else(|| {
                    (measured_voltage_error > VOLTAGE_READBACK_TOLERANCE_KV
                        || measured_current_error > CURRENT_READBACK_TOLERANCE_UA)
                        .then(|| format!(
                            "Moxtek measured telemetry outside advisory limits during enable: requested={requested_voltage:.3} kV/{requested_current:.3} uA, measured={:.3} kV/{:.3} uA, errors={measured_voltage_error:.3} kV/{measured_current_error:.3} uA",
                            status.voltage_kv, status.current_ua
                        ))
                });
                self.health.last_error = None;
                return Ok(());
    ...
    fn apply_health(&mut self, setpoints: &Setpoints, status: &Status) {
        ...
        self.health.current_ua = Some(status.current_ua);
        self.health.temperature_c = Some(status.temperature_c);
        self.health.telemetry_warning = None;
        self.health.last_error = None;
    }
    ...
    fn set_usb_auto_shutdown_state(&mut self, enabled: bool) -> Result<(), XrayError> {
...
fn transact_transport(
    ...
    let mut transport = transport
        .lock()
        .map_err(|_| XrayError::Transport("Moxtek transport lock was poisoned".into()))?;
...
fn is_transport_timeout(error: &XrayError) -> bool {
    matches!(error, XrayError::Transport(message) if {
...
mod tests {
    ...
    impl BinaryTransport for FakeTransport {
        ...
        fn write_all_bytes(&mut self, bytes: &[u8]) -> io::Result<()> {
            ...
            if command == CMD_SET_XRAY_ENABLE && bytes[3] != 0 && state.timeout_enable_ack {
                state.timeout_enable_ack = false;
        ...
        fn read_exact_bytes(&mut self, bytes: &mut [u8]) -> io::Result<()> {
            {
                ...
                if state.next_read_timeout {
                    ...
                    state.response = None;
                    return Err(io::Error::new(io::ErrorKind::TimedOut, "enable ACK timeout"));
                }
            }
            let response = self
                .lock()
    ...
    #[test]
    fn connect_forces_off_a_beam_left_on_by_a_killed_process() {
    ...
    #[test]
    fn beam_enable_preserves_explicit_auto_shutdown_state_and_verifies_readback() {
        let state = Arc::new(Mutex::new(FakeState::default()));
        let mut adapter = connected_adapter(state.clone());
        ...
        assert!(fake.enabled);
        assert!(!fake.usb_auto_shutdown);
        let auto_shutdown_off_index = fake
            .writes
            .iter()
            .rposition(|packet| {
                packet.as_slice() == [PACKET_HEADER, CMD_USB_AUTO_SHUTDOWN, 1, 0]
            })
            .unwrap();
        let beam_on_index = fake
            .writes
            .iter()
            .rposition(|packet| {
                packet.as_slice() == [PACKET_HEADER, CMD_SET_XRAY_ENABLE, 1, 1]
            })
            .unwrap();
        assert!(auto_shutdown_off_index < beam_on_index);
        drop(fake);
    ...
    #[test]
    fn beam_enable_accepts_missing_ack_only_after_independent_state_verification() {
        let state = Arc::new(Mutex::new(FakeState::default()));
        let mut adapter = connected_adapter(state.clone());
        adapter.set_parameters(60.0, 100.0).unwrap();
        adapter.last_off = Some(Instant::now() - MIN_REENABLE_INTERVAL);
        state.lock().unwrap().timeout_enable_ack = true;
    ...
    fn beam_enable_rejects_case_temperature_above_65_c() {
        let state = Arc::new(Mutex::new(FakeState {
            temperature_raw: (66.0 * TEMP_SCALE + TEMP_OFFSET).round() as u16,
            ..FakeState::default()
        }));
        let mut adapter = connected_adapter(state.clone());
        adapter.set_parameters(60.0, 200.0).unwrap();
        ...
        let cancel = AtomicBool::new(false);
        let result = adapter.beam_on(&cancel);

        assert!(matches!(
            &result,
            Err(XrayError::Safety(message)) if message.contains("exceeds 65")
        ));
        let fake = state.lock().unwrap();
        assert!(!fake.enabled);
    ...
    #[test]
    fn active_emission_temperature_fault_forces_output_off() {
        let state = Arc::new(Mutex::new(FakeState::default()));
        let mut adapter = connected_adapter(state.clone());
        adapter.set_parameters(60.0, 200.0).unwrap();
        adapter.last_off = Some(Instant::now() - MIN_REENABLE_INTERVAL);
        let cancel = AtomicBool::new(false);
        adapter.beam_on(&cancel).unwrap();

        state.lock().unwrap().temperature_raw =
            (66.0 * TEMP_SCALE + TEMP_OFFSET).round() as u16;
        let result = adapter.refresh_emission_status();

        assert!(matches!(
            &result,
            Err(XrayError::Safety(message)) if message.contains("during emission")
        ));
        assert!(!state.lock().unwrap().enabled);
        assert!(adapter.health().beam_off_confirmed);
    }
    ...
    #[test]
    fn active_emission_reports_measured_current_drift_without_stopping_output() {
        let state = Arc::new(Mutex::new(FakeState::default()));
        let mut adapter = connected_adapter(state.clone());
        adapter.set_parameters(60.0, 100.0).unwrap();
        adapter.last_off = Some(Instant::now() - MIN_REENABLE_INTERVAL);
        ...
        state.lock().unwrap().measured_current_raw = ua_to_raw(95.1).unwrap();
        assert!(adapter.refresh_emission_status().is_ok());
        assert!(state.lock().unwrap().enabled);

        state.lock().unwrap().measured_current_raw = ua_to_raw(94.9).unwrap();
        let health = adapter.refresh_emission_status().unwrap();
        assert!(health.telemetry_warning.as_deref().is_some_and(|message| {
            message.contains("measured telemetry outside advisory limits")
                && message.contains("advisory limits 0.300 kV/5.000 uA")
        }));
        assert!(state.lock().unwrap().enabled);
        assert!(health.beam_on && !health.beam_off_confirmed);
    }
    ...
    #[test]
    fn active_emission_reports_measured_voltage_drift_without_stopping_output() {
        let state = Arc::new(Mutex::new(FakeState::default()));
        let mut adapter = connected_adapter(state.clone());
        adapter.set_parameters(60.0, 100.0).unwrap();
        adapter.last_off = Some(Instant::now() - MIN_REENABLE_INTERVAL);
        ...
        state.lock().unwrap().measured_voltage_raw = kv_to_raw(60.29).unwrap();
        assert!(adapter.refresh_emission_status().is_ok());
        assert!(state.lock().unwrap().enabled);

        state.lock().unwrap().measured_voltage_raw = kv_to_raw(60.31).unwrap();
        let health = adapter.refresh_emission_status().unwrap();
        assert!(health.telemetry_warning.as_deref().is_some_and(|message| {
            message.contains("measured telemetry outside advisory limits")
                && message.contains("advisory limits 0.300 kV/5.000 uA")
    ...
    #[test]
    fn active_emission_still_fails_closed_when_accepted_setpoint_changes() {
        let state = Arc::new(Mutex::new(FakeState::default()));
        let mut adapter = connected_adapter(state.clone());
        adapter.set_parameters(60.0, 100.0).unwrap();
        adapter.last_off = Some(Instant::now() - MIN_REENABLE_INTERVAL);
        ...
        state.lock().unwrap().set_current_raw = ua_to_raw(110.0).unwrap();
        let result = adapter.refresh_emission_status();
        assert!(matches!(
            &result,
            Err(XrayError::Safety(message))
                if message.contains("accepted setpoint changed during emission")
        ));
        assert!(!state.lock().unwrap().enabled);
        assert!(adapter.health().beam_off_confirmed);
    }
